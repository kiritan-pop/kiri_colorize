...
class KiriDcgan():
    def __init__(self, image_path, batch_size, GPUs, g_path, d_path, result_path='results/'):
        ...
        for f in os.listdir(image_path):
            self.images.append(os.path.join(image_path, f))
        random.shuffle(self.images)
        self.valid_images = random.sample(self.images, len(self.images)//10)
        for d in self.valid_images:
            self.images.remove(d)

        self.image_que = multiprocessing.Queue(4096)
        self.valid_image_que = multiprocessing.Queue(4096)
        def datagen(que, img_path):
            ...
            while True:
                for path in img_path:
                    #本物
                    img = image_arrange(path, resize=(128,128))
                    img = (np.asarray(img)-127.5)/127.5
                    que.put(img)

        multiprocessing.Process(target=datagen, args=(self.image_que, self.images)).start()
        multiprocessing.Process(target=datagen, args=(self.valid_image_que, self.valid_images)).start()



    def train(self, GPUs, start_idx, batch_size):
